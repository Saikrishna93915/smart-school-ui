...
const ExamsPage: React.FC = () => {
  ...
  const stats = useMemo(() => ({
    ...
    scheduled: filteredExams.filter(e => e.status?.toLowerCase() === 'scheduled').length,
    ongoing: filteredExams.filter(e => ['ongoing', 'live'].includes(e.status?.toLowerCase())).length,
    completed: filteredExams.filter(e => e.status?.toLowerCase() === 'completed').length,
    draft: filteredExams.filter(e => e.status?.toLowerCase() === 'draft').length,
  }), [filteredExams]);

  // Compute unique classes
  const uniqueClasses = useMemo(() => {
    const typedExams = Array.isArray(exams) ? exams as ExamWithSubmission[] : [];
    return Array.from(new Set(typedExams.filter(e => e?.className).map(e => e.className!).filter(Boolean).sort()));
  }, [exams]);
  ...
  const getRoleBasedMessage = () => {
    if (isTeacher) {
      ...
      return 'View and take your scheduled exams';
    } else {
  ...
  const getExamStatusBadge = (status: string = '') => {
    ...
    switch (statusLower) {
      case 'draft':
        return { text: 'Draft', className: 'bg-gray-100 text-gray-800', color: 'bg-gray-100' };
      case 'scheduled':
        return { text: 'Scheduled', className: 'bg-blue-100 text-blue-800', color: 'bg-blue-100' };
      case 'ongoing':
      case 'live':
        return { text: 'Active', className: 'bg-green-100 text-green-800', color: 'bg-green-100' };
      case 'completed':
        return { text: 'Completed', className: 'bg-purple-100 text-purple-800', color: 'bg-purple-100' };
      case 'archived':
        return { text: 'Archived', className: 'bg-gray-100 text-gray-800', color: 'bg-gray-100' };
      default:
        return { text: 'Unknown', className: 'bg-gray-100 text-gray-800', color: 'bg-gray-100' };
    }
  };
  ...
  const renderExamList = () => (
    <div className="space-y-3">
      {filteredExams.map((exam) => {
        ...
        const canTakeExam = isStudent && (exam.status?.toLowerCase() === 'ongoing' || 
                                        exam.status?.toLowerCase() === 'live' ||
              ...
              <div className="flex-1">
                <div className="flex items-start gap-3">
                  <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${statusBadge.color}`}>
                    ...
                    ) : exam.status?.toLowerCase() === 'ongoing' || exam.status?.toLowerCase() === 'live' ? (
