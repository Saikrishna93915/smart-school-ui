...
const TakeExam: React.FC = () => {
  ...
  const warningFlagsRef = useRef({ tenMinute: false, fiveMinute: false, oneMinute: false, sixtySecond: false, tenSecond: false });
  ...
  }, [answers, examId, navigate, releaseMediaStream, submitting]);

  const handleAutoSubmit = useCallback((reason: string) => {
    performSubmit(reason, true);
  }, [performSubmit]);

  // Note: All audio/sound functionality has been removed from the exam system

  const handleRetest = useCallback(() => {
    console.log('🔄 Retest initiated - Clearing previous submission state');
    // Reset all exam state to allow retaking
    ...
    warningFlagsRef.current = { tenMinute: false, fiveMinute: false, oneMinute: false, sixtySecond: false, tenSecond: false };
    autoSubmitInProgressRef.current = false;
  }, []);

  // Load exam and check for previous attempts on mount
  useEffect(() => {
      ...
      try {
        ...
        try {
          ...
          if (submissionsResponse.success && submissionsResponse.data) {
            const existingSubmission = (submissionsResponse.data as Submission[]).find(
              (sub: Submission) =>
                typeof sub.exam === 'string' ? sub.exam === examId : sub.exam?._id === examId
            );
            
            if (existingSubmission) {
              console.log('Student has already attempted this exam', existingSubmission);
              setAlreadyAttempted(true);
              setPreviousSubmission(existingSubmission);
              
              // Still load exam data but don't allow retake
              if (typeof existingSubmission.exam !== 'string' && existingSubmission.exam) {
                examData = existingSubmission.exam as Exam;
              }
            }
          }
        } catch (err: any) {
          console.warn('Failed to check previous submissions', err);
        }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeRemaining((prev) => {
        ...
        if (next === 60 && !warningFlagsRef.current.sixtySecond) {
          warningFlagsRef.current.sixtySecond = true;
          showWarning('60 Seconds Left', '60 seconds remaining to complete the exam.');
        }

        if (next === 10 && !warningFlagsRef.current.tenSecond) {
          warningFlagsRef.current.tenSecond = true;
          showWarning('Final 10 Seconds', 'Only 10 seconds remaining!');
        }
  ...
  }, [examStarted, handleAutoSubmit, showWarning, timeRemaining, timerStarted]);
  ...
  // ONE ATTEMPT RULE: Block retake if already attempted
  if (alreadyAttempted) {
    return (
      <div className="flex items-center justify-center min-h-screen p-4 bg-gray-50">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              <p className="text-sm text-amber-800 font-medium mb-2">✓ Attempt Already Completed</p>
              <p className="text-sm text-amber-700">
                You have already completed this exam. Each exam can only be attempted once.
              </p>
              {previousSubmission && (
                <div className="mt-3 text-xs text-amber-700 space-y-1 border-t border-amber-200 pt-3">
                  <div><strong>Submitted:</strong> {new Date(previousSubmission.submittedAt || '').toLocaleString()}</div>
                  <div><strong>Status:</strong> {previousSubmission.status}</div>
                  {previousSubmission.totalMarksObtained !== undefined && (
                    <div><strong>Marks:</strong> {previousSubmission.totalMarksObtained}</div>
                  )}
                </div>
              )}
            </div>
          ...
          <Card className="w-full max-w-2xl">
            ...
            <CardContent className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-900 font-medium mb-2">⚠️ Important Notice</p>
                <p className="text-sm text-blue-800">
                  This exam allows only ONE attempt. Once you submit, you cannot retake it.
                </p>
              </div>

              <div className="text-sm text-muted-foreground space-y-2">
                <p>Security rules active during the exam:</p>
  ...
  return (
    ...
    <ExamWarningModal
      ...
      title="Submit Exam"
      message="Are you sure you want to submit? You cannot change your answers after submission. This is your ONLY attempt."
